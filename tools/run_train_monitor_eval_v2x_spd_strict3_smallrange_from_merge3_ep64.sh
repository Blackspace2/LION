...
PRETRAINED_MODEL="${PRETRAINED_MODEL:-/root/project/LION/run/cfgs/kitti_models/second_with_lion_mamba_64dim_v2x_spd_merge3_trainvox25000/v2x_spd_merge3_lion_mamba_fp32_bs2_trainvox25000/ckpt/checkpoint_epoch_64.pth}"
...
if [[ "${SMOKE_ONLY}" == "1" ]]; then
  echo "========== smoke-only pretrained forward/backward step =========="
  python - <<'PY'
import os
import logging
import torch
...
while (( current_epoch < EPOCHS )); do
  ...
  echo "========== train to epoch ${next_epoch} =========="
  train_cmd=(
  ...
  if ! find "${CKPT_DIR}" -maxdepth 1 -type f -name '*.pth' | grep -q .; then
