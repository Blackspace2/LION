...
Important env vars:
  ...
  FIX_RANDOM_SEED=1 by default, RANDOM_SEED=666 by default
  EVAL_TAG, EVAL_BATCH_SIZE, EVAL_WORKERS, CKPT_PATH, CKPT_DIR
  START_EPOCH, END_EPOCH, SKIP_EXISTING, GENERATE_TABLES, TABLE_METRICS=3d
...
run_train_once() {
  ...
  if [[ "${FIX_RANDOM_SEED}" == "1" ]]; then
    cmd+=(--fix_random_seed --seed "${RANDOM_SEED}")
  fi
